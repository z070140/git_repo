...
//    String key = (request.getParameter("key")!=null)? request.getParameter("key"):(String) session.getAttribute("key");
    if (key == null) key = "";
    String condition = "";
    ...
    if (class_id == null || "".equals(class_id)) {
        ...
        if (!"".equals(key)) {
            ...
            masterCondition += " student_id='" + userName + "' and note_content like '%" + key + "%'";
            System.out.println(condition);

        } else {
            condition += "student_id='" + userName + "'";
            masterCondition += "student_id='" + userName + "'";
        }
        ...
        if (!"".equals(key)) {
            condition += "and student_id='" + userName + "' and note_content like '%" + key + "%'";
            masterCondition += "and student_id='" + userName + "' and note_content like '%" + key + "%'";
            System.out.println(condition);
...
<a href="studentSearch.jsp?page=<%=pageNum-1==0 ? 1 : pageNum-1%>">上一页</a>
<%
    for (int i = 1; i <= totalPage; i++) {
        if (i == pageNum) {
%>
<a style="text-decoration: black;color: red" href="studentSearch.jsp?page=<%=i%>"><%=i%>&nbsp;&nbsp;&nbsp;
</a>
<%
} else {
%>
<a style="text-decoration: none" href="studentSearch.jsp?page=<%=i%>"><%=i%>&nbsp;&nbsp;&nbsp;
</a>
<%
        }
    }
...
<a href="studentSearch.jsp?page=<%=pageNum+1>totalPage ? totalPage : pageNum+1%>">下一页</a>
</body>
</html>