...
<form name="form1" method="post" action="">
    关键字：<input name="key1" type="text"> <input type="submit" name="Submit" value="查找">
</form>
...
<%
    String[] field = {"id"};
    List dataList = dob.getData("book", field, !"".equals(key) ? "author like '%" + key + "%'" : "");
...
<table width="551" border="0" cellpadding="0" cellspacing="1" bgcolor="#999999">
    ...
    <tr>
        ...
        <td bgcolor="#FFFFFF"><a href="detail.jsp?id=<%=ss[0]%>" target=_blank>查看</a>/<a
                href="modify.jsp?id=<%=ss[0]%>">修改</a>/<a href="delete.do?id=<%=ss[0] %>">删除</a></td>
    </tr>
    <% } %>
</table>
<%
...
%>
<a href="search.jsp?page=<%=pageNum-1==0 ? 1 : pageNum-1%>&key=<%=key%>">上一页</a>
<a href="search.jsp?page=<%=pageNum+1>totalPage ? totalPage : pageNum+1%>&key=<%=key%>">下一页</a>
</body>
</html>
